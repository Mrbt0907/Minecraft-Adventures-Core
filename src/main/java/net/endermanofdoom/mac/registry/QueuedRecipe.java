...
public class QueuedRecipe
{
	...
	public QueuedRecipe(Object output, int amount, int meta, String id, String pattern, QueuedIngredient... ingredients)
	{
		...
		if (type == EnumIngredientType.INVALID || type == EnumIngredientType.OREDICT)
			MACCore.error("Tried to register a recipe with an invalid ingredient type \"" + output.getClass().getCanonicalName() + "\"");
		this.output = output;
		this.amount = amount;
		this.meta = meta;
		...
		if (length <= 0)
			MACCore.error("Tried to register a recipe with that does not have enough valid ingredients \"" + String.valueOf(output) + "\"");
	...
	public ItemStack build()
	{
		switch (type)
		{
			...
			case OREDICT:
				MACCore.fatal("Tried to build an oredict as an output for a recipe \"" + output + "\"");
				return null;
			default:
