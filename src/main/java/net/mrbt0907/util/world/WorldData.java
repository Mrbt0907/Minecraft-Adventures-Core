...
import net.mrbt0907.util.util.EnumType;
import net.mrbt0907.util.util.FileUtil;
...
public class WorldData
{
	...
	public WorldData(String fileName)
	{
		if (!WorldDataManager.isAvailable())
			MrbtAPI.fatal("Cannot create world data when the world has not started");
		if (WorldDataManager.exists(fileName))
			MrbtAPI.fatal("Created duplicate world data objects");
	...
	public void save()
	{
		if (!WorldDataManager.exists(fileName))
		{
			MrbtAPI.error("World Data " + fileName + " is not initialized. Create a new world data object");
			return;
		}
		if (WorldDataManager.isRemote())
		{
			MrbtAPI.error("Cannot save world data " + fileName + " on the client");
			return;
		}
	...
	public void setBoolean(String key, boolean value)
	{
		if (WorldDataManager.isRemote())
		{
			MrbtAPI.error("Cannot set world data " + fileName + " on the client");
			return;
		}
